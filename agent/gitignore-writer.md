...
## Workflow
1. **Receive a prompt** from the calling agent (typically the Gitter). The prompt will ask you to check and update `.gitignore`.
...
2. **Add patterns at the end** of the file, grouped by category with comments. You do not need to ignore the files that are not inlcuded in the project folder.
