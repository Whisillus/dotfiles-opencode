...
- Strict mode uses `set -Eeuo pipefail` with a brief “why” explanation for each flag.
...
- Logging functions exist with these names and behaviors:
  ...
  - Avoid logging full failed command lines by default, to reduce secret leakage in stderr and log files
- Cleanup pattern exists:
  - `cleanup()` function
  - An `EXIT` trap wrapper that preserves the original status before calling `cleanup()`
...
- When sourced, the script must not auto-run `main` and must not `exit` the caller shell. Calling `main` while sourced must return safely or refuse execution.
...
- `3`: refused sourced execution of `main()`.
- `4`: missing dependency (required command not found).
- `130`: interrupted by SIGINT.
- `143`: terminated by SIGTERM.
- On unexpected command failure under strict mode: exit with the failing command’s status (via `ERR` trap) without echoing the full failed command line by default.

## Verification Checklist

- [ ] Template contains required metadata and `usage()` block.
- [ ] `set -Eeuo pipefail` exists with explanation comments.
...
- [ ] Script does not auto-run and does not `exit` when sourced; calling `main` while sourced returns safely.
- [ ] Error logs avoid writing full failed command lines by default.
